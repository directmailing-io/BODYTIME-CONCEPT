...
type SortKey = 'name' | 'contract_end_date' | 'order_date' | 'partner';

export default function AdminCustomersTable({ customers }: { customers: any[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    // Search by customer name, email, or partner name/email
    if (search.trim()) {
      const q = search.toLowerCase();
      list = list.filter(c => {
        const partnerName = `${c.partner?.first_name ?? ''} ${c.partner?.last_name ?? ''}`.toLowerCase();
        const partnerEmail = c.partner?.email?.toLowerCase() ?? '';
        return (
          `${c.first_name} ${c.last_name}`.toLowerCase().includes(q) ||
          c.email?.toLowerCase().includes(q) ||
          partnerName.includes(q) ||
          partnerEmail.includes(q)
        );
      });
    }
    ...
    list = [...list].sort((a, b) => {
      ...
      if (sortKey === 'partner') {
        av = `${a.partner?.first_name ?? ''} ${a.partner?.last_name ?? ''}`;
        bv = `${b.partner?.first_name ?? ''} ${b.partner?.last_name ?? ''}`;
      }
                ...
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  <th className="text-left px-4 py-3 font-medium text-gray-500">
                    <button className="flex items-center gap-1 hover:text-gray-900" onClick={() => toggleSort('name')}>
                      Kunde <ArrowUpDown className="h-3 w-3" />
                    </button>
                  </th>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-gray-500 hidden lg:table-cell">
                    <button className="flex items-center gap-1 hover:text-gray-900" onClick={() => toggleSort('partner')}>
                      Partner <ArrowUpDown className="h-3 w-3" />
                    </button>
                  </th>
                  <th className="text-left px-4 py-3 font-medium text-gray-500 hidden lg:table-cell">
                      ...
                      <td className="px-4 py-3 hidden lg:table-cell">
                        {customer.partner_id ? (
                          <Link
                            ...
                            className="group flex flex-col hover:text-gray-900 transition-colors"
                            onClick={e => e.stopPropagation()}
                          >
                            <span className="text-gray-700 group-hover:underline">{partnerName}</span>
                            {customer.partner?.email && (
                              <span className="text-xs text-gray-400">{customer.partner.email}</span>
                            )}
                          </Link>
                        ) : (
                        ...
                        <div className="flex flex-col">
                          <span className={
                            isExpired(customer.contract_end_date)
                              ...
                              : isExpiringSoon(customer.contract_end_date)
