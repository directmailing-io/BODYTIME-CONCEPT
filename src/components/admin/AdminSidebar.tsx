...
// Mobile — 2 left + [+] center + 2 right = 5 slots
const leftTabs  = [
  { href: '/admin/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/admin/partners',  label: 'Partner',   icon: Users },
];
      ...
      <div className="px-3 py-4 border-t border-gray-100">
        <div className="flex items-center gap-3 px-3 py-2 mb-1">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium text-gray-900 truncate">{[profile.first_name, profile.last_name].filter(Boolean).join(' ') || 'Admin'}</p>
...
function MobileNav({ profile, newReferralCount = 0 }: { profile: AdminProfile; newReferralCount?: number }) {
  ...
  const isEmpfehlungActive = pathname === '/admin/empfehlungszentrale' || pathname.startsWith('/admin/empfehlungszentrale/');
    ...
    <>
      ...
      >
        <div className="flex items-stretch h-16">
          ...
          {/* Center — blue circle FAB, no label */}
          <button
            onClick={() => setCreateOpen(true)}
            className="flex-1 flex items-center justify-center"
            aria-label="Partner anlegen"
          >
            <div className="w-12 h-12 rounded-full bg-blue-500 flex items-center justify-center shadow-md shadow-blue-500/30 active:scale-95 transition-transform">
          ...
          {/* Mehr */}
          <button onClick={() => setSheetOpen(true)} className="flex-1 flex flex-col items-center justify-center gap-1 min-w-0 relative">
            <div className="relative">
              ...
              {/* Orange dot when new referrals are in Mehr (not visible on the primary Empfehlung tab) */}
              {newReferralCount > 0 && !isEmpfehlungActive && (
      ...
      {/* Controlled CreatePartnerDialog */}
        ...
        <div className="flex items-center justify-between px-5 pt-4 pb-2 relative">
          ...
          <div className="flex items-center gap-3 pt-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-gray-900 leading-snug">{[profile.first_name, profile.last_name].filter(Boolean).join(' ') || 'Admin'}</p>
          ...
          {secondaryItems.map(({ href, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                ...
                <span className={['text-[15px] flex-1', isActive ? 'font-semibold' : 'font-medium'].join(' ')}>{label}</span>
              </Link>
            );
          })}
        </div>
